...
- 优点：
  - 相比于其它的数据结构，布隆过滤器在空间和时间方面都有巨大的优势。布隆过滤器存储空间和插⼊/查询时间都是常数 O(K)，另外，散列函数相互之间没有关系，⽅便由硬件并⾏实现。布隆过滤器不需要存储元素本身，在某些对保密要求非常严格的场合有优势。
...
本地差分隐私：认为第三方是不可信的，所以本地差分隐私保护的是⽤户上传数据到第三方的过程，差分隐私
